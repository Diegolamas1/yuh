...
import rospy
import os
from Project_2.msg import Alignment
t_vecs_aruco = []
d            = []
theta        = []
...
def pose_estimation(frame, intric_matrix, distortion):
    ...
    detected_markers, theta = aruco_show(corners, ids, rejected, frame)
...
## Publish the message ##
#########################
def navigation(theta, d):
    # define the actions the publisher will make
    pub = rospy.Publisher("/Alignment", Alignment, queue_size=10)
...
files_and_dirs = os.listdir(imgdir)
imagenames = [file for file in files_and_dirs if file.endswith('.png')]
imagepaths = []

for im in imagenames:
	path = os.path.join(imgdir, im)
...
ret, intric_matrix, distortion, r_matrix, t_vecs = cv2.calibrateCamera(points_3D,
    points_2D, last_gray_shape, None, None)
##################################################################################
...
while video.isOpened():
    ...
    pose, d, theta = pose_estimation(frame, intric_matrix, distortion)
